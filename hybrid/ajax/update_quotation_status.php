...
$sql = mysqli_query($conn,"UPDATE table_quotations SET `quotation_status` = '$status', `agent_remarks` = '$remarks', `status_changed_date` = '$today_date' WHERE id = '$Id'");

// echo "Quotation Status Updated Successfully!!!";

echo "UPDATE table_quotations SET `quotation_status` = '$status', `agent_remarks` = '$remarks', `status_changed_date` = '$today_date' WHERE id = '$Id'";

}else{

$sql7 = mysqli_query($conn,"SELECT quotation_id,customer_name,customer_email,customer_mobile,customer_address,service,scope_of_work,quotation_rate FROM `table_quotations` WHERE id = '$Id'");

$row = mysqli_fetch_assoc($sql7);
	
$quotation_id 		= $row['quotation_id'];
$customer_name 		= mysqli_real_escape_string($conn,$row['customer_name']);
...
$sql3 = mysqli_query($conn,"UPDATE `table_quotations` SET `quotation_status` = '$status', `job_scheduled` = 'Yes', `agent_remarks` = '$remarks', `status_changed_date` = '$today_date' WHERE `id` = '$Id'");

// echo "Job Scheduled Successfully!!!";

echo "INSERT INTO `table_jobs` ( `quotation_id`,`job_sheet_id`,`cust_name`,`cust_mobile`,`cust_email`,`cust_address`,`service`, `booking_date`,`start_date`,`end_date`, `start_time`, `end_time`,`technician`,`job_to_do`,`tools_required`,`spare_parts`, `agent_name`, `agent_remark`, `job_sheet_generated`, `created_date`, `job_sheet_generated_date`, `jobsheet_status`, `job_status_changed_date`,`job_amount`) VALUES ( '$quotation_id','$job_sheet_id','$customer_name','$customer_mobile','$customer_email','$customer_address','$service','$service_date','$service_date','','$start_time','$end_time','','$scope_of_work','','','$agent_name','','No','$today_date','','','','$job_amount' )";
